...
export default function GridPage() {
  ...
  const [prizes, setPrizes] = useState({ q1: 250, q2: 250, q3: 250, q4: 750 });
  ...
  const loadData = async () => {
    try {
      ...
      const { data: settings } = await supabase
        .from('settings')
        .select('key, value')
        .in('key', ['pool_active', 'square_price', 'prize_q1', 'prize_q2', 'prize_q3', 'prize_q4']);

      if (settings) {
        const poolSetting = settings.find(s => s.key === 'pool_active');
        const priceSetting = settings.find(s => s.key === 'square_price');
        const q1 = settings.find(s => s.key === 'prize_q1');
        const q2 = settings.find(s => s.key === 'prize_q2');
        const q3 = settings.find(s => s.key === 'prize_q3');
        const q4 = settings.find(s => s.key === 'prize_q4');

        if (poolSetting) setPoolActive(poolSetting.value === 'true');
        if (priceSetting?.value) setSquarePrice(parseFloat(priceSetting.value) || 50);
        setPrizes({
          q1: q1?.value ? parseFloat(q1.value) : 250,
          q2: q2?.value ? parseFloat(q2.value) : 250,
          q3: q3?.value ? parseFloat(q3.value) : 250,
          q4: q4?.value ? parseFloat(q4.value) : 750,
        });
      }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-6">
          ...
          <div className="space-y-4">
            {/* Prize Info Bar - Collapsed by default */}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-base text-gray-500">Win at each quarter end</div>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-3xl font-black text-[#d4af37]">${totalPrizePool.toLocaleString()}</span>
                  <svg className={`w-5 h-5 text-gray-400 transition-transform ${showInfo ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="px-5 pb-5 pt-2 border-t border-gray-200">
                      ...
                      <div className="grid grid-cols-4 gap-3 mb-4">
                        {[
                          ...
                          <div key={q} className="text-center p-3 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="space-y-2 text-base text-gray-600">
                        <div className="flex items-start gap-2">
                          <span className="text-[#30d158] font-bold">1.</span>
                          <span>Pick squares on the grid (${squarePrice} each)</span>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="text-[#30d158] font-bold">2.</span>
                          <span>Numbers 0-9 assigned randomly to rows/columns before kickoff</span>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="text-[#30d158] font-bold">3.</span>
                          <span>Match last digit of both scores at quarter end to win!</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
